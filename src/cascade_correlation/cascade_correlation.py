...
from cascade_correlation.cascade_correlation_exceptions.cascade_correlation_exceptions import (  # CascadeCorrelationError,; NetworkInitializationError,
    ConfigurationError,
    TrainingError,
    ValidationError,
)
...
class CascadeCorrelationNetwork:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Logger.debug(
            "CascadeCorrelationNetwork: __init__: Initializing Cascade Correlation Network"
        )
        super().__init__()
    ...
    def _init_config(self, config: CascadeCorrelationConfig = None) -> None:
        ...
        if hasattr(self, "logger") and self.logger is not None:
            ...
            logger = Logger
        logger.debug(
            "CascadeCorrelationNetwork: _init_config: Initializing configuration"
        )
        if config is None:
            config = CascadeCorrelationConfig()
        self.config = config
        logger.debug(
            f"CascadeCorrelationNetwork: _init_config: Configuration set to: {self.config}"
        )
        self.set_uuid(self.config.uuid)
        logger.debug(
            f"CascadeCorrelationNetwork: _init_config: UUID set to: {self.uuid}"
        )

    def _init_logging_system(self) -> None:
        """Initialize the logging system with proper configuration."""
        Logger.debug(
            "CascadeCorrelationNetwork: _init_logging_system: Initializing logging system"
        )

        # Set up log parameters
        self.log_file_name = (
            self.config.log_file_name
            or _CASCADE_CORRELATION_NETWORK_LOG_FILE_NAME
            or __name__
        )
        self.log_file_path = (
            self.config.log_file_path
            or _CASCADE_CORRELATION_NETWORK_LOG_FILE_PATH
            or str(os.path.join(os.getcwd(), "logs"))
        )
        self.log_level_name = (
            self.config.log_level_name or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_NAME
        )
        ...
        self.log_config = self.config.log_config or LogConfig(
            ...
            _LogConfig__log_date_format=self.config.log_date_format
            or _CASCADE_CORRELATION_NETWORK_LOG_DATE_FORMAT,
            _LogConfig__log_format_string=self.config.log_format_string
            or _CASCADE_CORRELATION_NETWORK_LOG_FORMATTER_STRING,
            _LogConfig__log_level_custom_names_list=self.config.log_level_custom_names_list
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_CUSTOM_NAMES_LIST,
            _LogConfig__log_level_methods_dict=self.config.log_level_methods_dict
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_METHODS_DICT,
            _LogConfig__log_level_methods_list=self.config.log_level_methods_list
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_METHODS_LIST,
            _LogConfig__log_level_names_list=self.config.log_level_names_list
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_NAMES_LIST,
            _LogConfig__log_level_numbers_dict=self.config.log_level_numbers_dict
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_NUMBERS_DICT,
            _LogConfig__log_level_numbers_list=self.config.log_level_numbers_list
            or _CASCADE_CORRELATION_NETWORK_LOG_LEVEL_NUMBERS_LIST,
        )
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _init_logging_system: Logger initialized with level: {self.logger.level}"
        )

    def _init_network_parameters(self) -> None:
        """Initialize network parameters, randomness, and model components."""
        Logger.debug(
            "CascadeCorrelationNetwork: _init_network_parameters: Initializing network parameters"
        )

        # Initialize randomness
        self.random_seed = (
            self.config.random_seed or _CASCADE_CORRELATION_NETWORK_RANDOM_SEED
        )
        self.random_max_value = (
            self.config.random_max_value
            or _CASCADE_CORRELATION_NETWORK_RANDOM_MAX_VALUE
        )
        self.sequence_max_value = (
            self.config.sequence_max_value
        ...
        self.input_size = (
            self.config.input_size or _CASCADE_CORRELATION_NETWORK_INPUT_SIZE
        )
        self.output_size = (
            self.config.output_size or _CASCADE_CORRELATION_NETWORK_OUTPUT_SIZE
        )
        self.candidate_pool_size = (
            self.config.candidate_pool_size
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_POOL_SIZE
        )

        # Initialize training parameters
        self.learning_rate = (
            self.config.learning_rate or _CASCADE_CORRELATION_NETWORK_LEARNING_RATE
        )
        self.candidate_learning_rate = (
            self.config.candidate_learning_rate
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_LEARNING_RATE
        )
        self.max_hidden_units = (
            self.config.max_hidden_units
            or _CASCADE_CORRELATION_NETWORK_MAX_HIDDEN_UNITS
        )
        self.correlation_threshold = (
            self.config.correlation_threshold
            or _CASCADE_CORRELATION_NETWORK_NODE_CORRELATION_THRESHOLD
        )
        ...
        self.candidate_epochs = (
            ...
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_EPOCHS
        )
        self.epochs_max = (
            self.config.epochs_max or _CASCADE_CORRELATION_NETWORK_EPOCHS_MAX
        )
        self.output_epochs = (
            self.config.output_epochs or _CASCADE_CORRELATION_NETWORK_OUTPUT_EPOCHS
        )
        self.random_value_scale = (
            self.config.random_value_scale
            or _CASCADE_CORRELATION_NETWORK_RANDOM_VALUE_SCALE
        )
        self.target_accuracy = (
            self.config.candidate_training_target_accuracy
            or _CASCADE_CORRELATION_NETWORK_TARGET_ACCURACY
        )
        self.worker_standby_sleepytime = (
            self.config.candidate_training_worker_standby_sleepytime
            or _CASCADE_CORRELATION_NETWORK_WORKER_STANDBY_SLEEPYTIME
        )
        self.shutdown_timeout = (
            self.config.candidate_training_shutdown_timeout
            or _CASCADE_CORRELATION_NETWORK_SHUTDOWN_TIMEOUT
        )
        self.task_queue_timeout = (
            self.config.candidate_training_task_queue_timeout
            or _CASCADE_CORRELATION_NETWORK_TASK_QUEUE_TIMEOUT
        )
        ...
        self.output_weights = (
            torch.randn(
                self.config.input_size, self.config.output_size, requires_grad=True
            )
            * self.random_value_scale
        )
        self.output_bias = (
            torch.randn(self.config.output_size, requires_grad=True)
            * self.random_value_scale
        )
        self.history = {
            "train_loss": [],
        ...
        # Snapshot directory
        self.cascade_correlation_network_snapshots_dir = (
            self.config.cascade_correlation_network_snapshots_dir
            or _CASCADE_CORRELATION_NETWORK_HDF5_PROJECT_SNAPSHOTS_DIR
        )
        self.logger.debug(
            "CascadeCorrelationNetwork: _init_network_parameters: Network parameters initialized"
        )
    ...
    def _init_activation_function(self):
        ...
        self.activation_function_name = (
            ...
            or _CASCADE_CORRELATION_NETWORK_ACTIVATION_FUNCTION_NAME
        )
        self.activation_functions_dict = (
            self.config.activation_functions_dict
            or _CASCADE_CORRELATION_NETWORK_ACTIVATION_FUNCTIONS_DICT
        )
    ...
    def _init_multiprocessing(self) -> None:
        ...
        self.logger.trace(
            "CascadeCorrelationNetwork: _init_multiprocessing: Initializing multiprocessing components"
        )
        ...
        context_type = (
            self.config.candidate_training_context_type
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_TRAINING_CONTEXT
        )
        ...
        self.candidate_training_queue_authkey = (
            self.config.candidate_training_queue_authkey
        )
        self.candidate_training_queue_address = (
            self.config.candidate_training_queue_address
        )
        ...
        self.candidate_training_context = (
            mp.get_context(self.config.candidate_training_context_type)
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_TRAINING_CONTEXT
        )
        self.logger.debug(
            "CascadeCorrelationNetwork: _init_multiprocessing: Multiprocessing components initialized"
        )

    def _init_display_components(self) -> None:
        """Initialize display and plotting components."""
        self.logger.trace(
            "CascadeCorrelationNetwork: _init_display_components: Initializing display components"
        )

        # Initialize display parameters
        self.display_frequency = (
            self.config.display_frequency
            or _CASCADE_CORRELATION_NETWORK_CANDIDATE_DISPLAY_FREQUENCY
        )
        self.epoch_display_frequency = (
            self.config.epoch_display_frequency
            or _CASCADE_CORRELATION_NETWORK_EPOCH_DISPLAY_FREQUENCY
        )
        self.generate_plots = (
            self.config.generate_plots or _CASCADE_CORRELATION_NETWORK_GENERATE_PLOTS
        )
        self.status_display_frequency = (
            self.config.status_display_frequency
            or _CASCADE_CORRELATION_NETWORK_STATUS_DISPLAY_FREQUENCY
        )
        self.candidate_display_frequency = (
            self.config.candidate_display_frequency
            or _CASCADE_CORRELATION_NETWORK_DISPLAY_FREQUENCY
        )
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _init_display_components: Display components initialized"
        )
    ...
    def _validate_tensor_input(
        ...
        if x.numel() > 0:
            ...
            if torch.isinf(x).any():
                raise ValidationError(
                    f"Parameter '{param_name}' contains infinite values"
                )
    ...
    def _validate_tensor_shapes(
        ...
        if len(x.shape) != 2:
            raise ValidationError(
                f"Input tensor must be 2D (batch_size, features), got shape {x.shape}"
            )
        if (
            expected_input_features is not None
            and x.shape[1] != expected_input_features
        ):
            raise ValidationError(
                f"Expected {expected_input_features} input features, got {x.shape[1]}"
            )
        if y is not None:
            if len(y.shape) != 2:
                raise ValidationError(
                    f"Target tensor must be 2D (batch_size, classes), got shape {y.shape}"
                )
            if x.shape[0] != y.shape[0]:
                raise ValidationError(
                    f"Input and target batch sizes must match: {x.shape[0]} != {y.shape[0]}"
                )
    ...
    def _validate_positive_integer(
        ...
        if not isinstance(value, int):
            raise ValidationError(
                f"Parameter '{param_name}' must be an integer, got {type(value)}"
            )
    ...
    def _create_candidate_unit(
        ...
        **kwargs,
        ...
        """
        self.logger.debug(
            f"CascadeCorrelationNetwork: _create_candidate_unit: Creating candidate unit {candidate_index}"
        )
        return CandidateUnit(
            ...
            CandidateUnit__random_value_scale=kwargs.get(
            ...
            CandidateUnit__sequence_max_value=kwargs.get(
                "sequence_max_value", self.sequence_max_value
            ),
            CandidateUnit__random_max_value=kwargs.get(
                "random_value_max", self.random_max_value
            ),
        )
    ...
    def _create_optimizer(self, parameters, config=None):
        ...
        config = config or getattr(
            self, "optimizer_config", OptimizerConfig(learning_rate=self.learning_rate)
        )
        optimizer_map = {
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _create_optimizer: Creating {config.optimizer_type} optimizer with LR={config.learning_rate}"
        )
    ...
    def fit(
        ...
        self._validate_tensor_shapes(
            x_train, y_train, expected_input_features=self.input_size
        )

        # Validate that target tensor has correct output size
        if y_train.shape[1] != self.output_size:
            raise ValidationError(
                f"Target tensor must have {self.output_size} output features, got {y_train.shape[1]}"
            )
        ...
        if y_val is not None:
            ...
            if x_val is None:
                raise ValidationError(
                    "CascadeCorrelationNetwork: fit: Cannot provide y_val without x_val"
                )
            self._validate_tensor_shapes(
                x_val, y_val, expected_input_features=self.input_size
            )
            if y_val.shape[1] != self.output_size:
                raise ValidationError(
                    f"CascadeCorrelationNetwork: fit: Validation target tensor must have {self.output_size} output features, got {y_val.shape[1]}"
                )
        ...
        if not isinstance(early_stopping, bool):
            raise ValidationError(
                f"CascadeCorrelationNetwork: fit: Parameter 'early_stopping' must be boolean, got {type(early_stopping)}"
            )
        if len(x_train) == 0:
            raise ValidationError(
                "CascadeCorrelationNetwork: fit: Training dataset cannot be empty"
            )
    ...
    def forward(self, x: torch.Tensor = None) -> torch.Tensor:
        ...
        self.logger.verbose(
            f"CascadeCorrelationNetwork: forward: Starting forward pass with input shape: {x.shape}"
        )
        features = x
        self.logger.debug(
    ...
    def train_output_layer(
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: train_output_layer: Created optimizer: {type(self.output_optimizer).__name__}"
        )
        ...
        if snapshot_path := self.create_snapshot() is not None:
            self.logger.info(
                f"CascadeCorrelationNetwork: train_output_layer: Created network snapshot at: {snapshot_path}"
            )
    ...
    def train_candidates(
        ...
        self.logger.verbose(
            f"CascadeCorrelationNetwork: train_candidates: Start time: {start_time}"
        )

        # Step 1: Prepare candidate input incorporating existing hidden units
        candidate_input = self._prepare_candidate_input(x)
        self.logger.debug(
            f"CascadeCorrelationNetwork: train_candidates: Prepared candidate input shape: {candidate_input.shape}"
        )

        # Step 2: Generate candidate training data and tasks
        tasks = self._generate_candidate_tasks(candidate_input, y, residual_error)
        self.logger.debug(
            f"CascadeCorrelationNetwork: train_candidates: Generated {len(tasks)} candidate training tasks."
        )

        # Step 3: Determine optimal process count for training
        process_count = self._calculate_optimal_process_count()
        self.logger.debug(
            f"CascadeCorrelationNetwork: train_candidates: Optimal process count for training: {process_count}"
        )

        # Step 4: Execute training (parallel or sequential)
        self.logger.trace(
            "CascadeCorrelationNetwork: train_candidates: Starting candidate training execution."
        )
        try:
            self.logger.info(
                f"CascadeCorrelationNetwork: train_candidates: Executing candidate training with {process_count} processes."
            )
            results = self._execute_candidate_training(tasks, process_count)
            self.logger.debug(
                f"CascadeCorrelationNetwork: train_candidates: Candidate training results: length: {len(results)}, value: {results}"
            )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: train_candidates: Error during candidate training: {e}"
            )
            import traceback
        ...
        self.logger.trace(
            f"CascadeCorrelationNetwork: train_candidates: Completed training of candidate units: Results: {results}."
        )

        # Step 5: Process and analyze results
        self.logger.trace(
            "CascadeCorrelationNetwork: train_candidates: Starting processing of candidate training results."
        )
        training_stats = self._process_training_results(results, tasks, start_time)
        self.logger.trace(
            f"CascadeCorrelationNetwork: train_candidates: Completed processing of candidate training results: {training_stats}."
        )
    ...
    def _prepare_candidate_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for unit in self.hidden_units:
            ...
            self.logger.debug(
                f"CascadeCorrelationNetwork: _prepare_candidate_input: Hidden unit output shape: {unit_output.shape}"
            )
        candidate_input = (
            torch.cat([x] + hidden_outputs, dim=1) if hidden_outputs else x
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: _prepare_candidate_input: Candidate input shape: {candidate_input.shape}"
        )
        self.logger.info(
            f"CascadeCorrelationNetwork: _prepare_candidate_input: Hidden units: {len(hidden_outputs)}"
        )
    ...
    def _generate_candidate_tasks(
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _generate_candidate_tasks: Created {len(tasks)} training tasks"
        )
        return tasks
    ...
    def _calculate_optimal_process_count(self) -> int:
        ...
        """
        self.logger.debug(
            f"CascadeCorrelationNetwork: _calculate_optimal_process_count: CPU count: {os.cpu_count()}"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: _calculate_optimal_process_count: Candidate pool size: {self.candidate_pool_size}"
        )
        ...
        if hasattr(os, "sched_getaffinity"):
            ...
            self.logger.debug(
                f"CascadeCorrelationNetwork: _calculate_optimal_process_count: Affinity CPU count: {affinity_cores}"
            )
        else:
        ...
        cpu_cores_available = min(
            ...
            affinity_cores,
            (
                self.candidate_training_context.cpu_count()
                if hasattr(self, "candidate_training_context")
                else os.cpu_count()
            ),
            os.cpu_count(),
        )
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _calculate_optimal_process_count: Using {process_count} processes"
        )
    ...
    def _execute_candidate_training(self, tasks: list, process_count: int) -> list:
        ...
        """
        self.logger.info(
            f"CascadeCorrelationNetwork: _execute_candidate_training: Training {len(tasks)} candidates with {process_count} processes"
        )
        results = []
        self.logger.debug(
            f"CascadeCorrelationNetwork: _execute_candidate_training: Adjusted process count to: {process_count}"
        )
        try:
            if process_count > 1:
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _execute_candidate_training: Using {process_count} processes"
                )
                ...
                if not results:
                    self.logger.warning(
                        "CascadeCorrelationNetwork: _execute_candidate_training: Parallel processing returned no results, falling back to sequential"
                    )
                    raise RuntimeError(
                        "CascadeCorrelationNetwork: _execute_candidate_training: Parallel processing failed to return results"
                    )
                self.logger.debug(
                    "CascadeCorrelationNetwork: _execute_candidate_training: Completed parallel processing"
                )
            else:
                self.logger.debug(
                    "CascadeCorrelationNetwork: _execute_candidate_training: Using sequential processing"
                )
                results = self._execute_sequential_training(tasks)
                self.logger.debug(
                    "CascadeCorrelationNetwork: _execute_candidate_training: Completed sequential processing"
                )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: _execute_candidate_training: Error in candidate node training: {e}"
            )
            import traceback

            self.logger.error(
                f"CascadeCorrelationNetwork: _execute_candidate_training: Traceback: {traceback.format_exc()}"
            )

            # Fall back to sequential training when parallel fails
            self.logger.warning(
                "CascadeCorrelationNetwork: _execute_candidate_training: Parallel training failed, falling back to sequential training"
            )
            try:
                results = self._execute_sequential_training(tasks)
                self.logger.info(
                    "CascadeCorrelationNetwork: _execute_candidate_training: Sequential training completed successfully"
                )
            except Exception as seq_error:
                self.logger.error(
                    f"CascadeCorrelationNetwork: _execute_candidate_training: Sequential training also failed: {seq_error}"
                )
                # Create dummy failure results for each task only if sequential also fails
                self.logger.warning(
                    "CascadeCorrelationNetwork: _execute_candidate_training: Creating dummy results for failed training"
                )
                results = self._get_dummy_results(len(tasks))
        self.logger.debug(
            f"CascadeCorrelationNetwork: _execute_candidate_training: Obtained {len(results)} results"
        )

        # Ensure we have some results:  For empty results list, create an intelligently empty dummy results
        if not results:
            self.logger.error(
                "CascadeCorrelationNetwork: _execute_candidate_training: No results obtained from either parallel or sequential processing"
            )
            results = self._get_dummy_results(len(tasks))
        return results
    ...
    def _execute_parallel_training(
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _execute_parallel_training: Using multiprocessing"
        )

        # Adjust process count if invalid
        process_count = (process_count, self._calculate_optimal_process_count())[
            process_count < 1
        ]
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _execute_parallel_training: Starting the manager server"
        )
        self._start_manager()
        # self.logger.trace("CascadeCorrelationNetwork: _execute_parallel_training: Manager server started")
        self.logger.debug(
            "CascadeCorrelationNetwork: _execute_parallel_training: Manager server started"
        )
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _execute_parallel_training: Created task and result queues"
        )
        try:
            # Add tasks to the queue
            # self.logger.trace("CascadeCorrelationNetwork: _execute_parallel_training: Adding tasks to the queue")
            self.logger.debug(
                "CascadeCorrelationNetwork: _execute_parallel_training: Adding tasks to the queue"
            )
            for task in tasks:
                task_queue.put(task)
            self.logger.debug(
                f"CascadeCorrelationNetwork: _execute_parallel_training: Added {len(tasks)} tasks to queue"
            )
            ...
            workers = []
            self.logger.debug(
                f"CascadeCorrelationNetwork: _execute_parallel_training: Starting {num_workers} workers"
            )

            for i in range(num_workers):
                # self.logger.trace(f"CascadeCorrelationNetwork: _execute_parallel_training: Starting worker {i}")
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _execute_parallel_training: Starting worker {i}"
                )
                ...
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _execute_parallel_training: Worker {i} instantiated."
                )
                # self.logger.trace("CascadeCorrelationNetwork: _execute_parallel_training: Starting worker process.")
                self.logger.debug(
                    "CascadeCorrelationNetwork: _execute_parallel_training: Starting worker process."
                )
                worker.start()
                # self.logger.trace(f"CascadeCorrelationNetwork: _execute_parallel_training: Worker {i} started with PID {worker.pid}.")
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _execute_parallel_training: Worker {i} started with PID {worker.pid}."
                )
                # self.logger.trace("CascadeCorrelationNetwork: _execute_parallel_training: Adding worker to the Workers list.")
                self.logger.debug(
                    "CascadeCorrelationNetwork: _execute_parallel_training: Adding worker to the Workers list."
                )
                ...
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _execute_parallel_training: Completed adding worker to the Workers list: workers: length: {len(workers)}, value: {workers}."
                )
            self.logger.debug(
                f"CascadeCorrelationNetwork: _execute_parallel_training: Successfully Started Workers: {len(workers)}."
            )
            self.logger.debug(
                f"CascadeCorrelationNetwork: _execute_parallel_training: Initial Queue Sizes: Task Queue: {task_queue.qsize()}, Result Queue: {result_queue.qsize()}"
            )
            ...
            self.logger.debug(
                "CascadeCorrelationNetwork: _execute_parallel_training: Collecting results from workers"
            )
            results = self._collect_training_results(result_queue, len(tasks))
            self.logger.debug(
                f"CascadeCorrelationNetwork: _execute_parallel_training: Collected {len(results)} results"
            )

            # Stop workers
            self._stop_workers(workers, task_queue)
            self.logger.debug(
                "CascadeCorrelationNetwork: _execute_parallel_training: Stopped all workers"
            )
        finally:
            self.logger.trace(
                "CascadeCorrelationNetwork: _execute_parallel_training: Stopping manager server"
            )
            self._stop_manager()
        return results
    ...
    def _execute_sequential_training(self, tasks: list) -> list:
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _execute_sequential_training: Using sequential processing"
        )
        results = []
        for candidate_index, task in enumerate(tasks):
            self.logger.verbose(
                f"CascadeCorrelationNetwork: _execute_sequential_training: Training candidate {candidate_index + 1}/{len(tasks)}"
            )
            try:
                candidate_training_result = self.train_candidate_worker(
                    task_data_input=task, parallel=False
                )
                results.append(candidate_training_result)
            except Exception as task_e:
                self.logger.error(
                    f"CascadeCorrelationNetwork: _execute_sequential_training: Task error: {task_e}"
                )
                results.append(
    ...
    def _collect_training_results(
        ...
        collected_results = 0
        self.logger.debug(
            f"CascadeCorrelationNetwork: _collect_training_results: Collecting {num_tasks} results"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: _collect_training_results: Timeout set to {queue_timeout} seconds"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: _collect_training_results: Result Queue: Length: {result_queue.qsize()}, Contents: {list(result_queue.queue) if hasattr(result_queue, 'queue') else 'N/A'}"
        )
        ...
        while collected_results < num_tasks and time.time() < deadline:
            try:
                ...
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _collect_training_results: Retrieved result: {result}"
                )
                results.append(result)
                collected_results += 1
                self.logger.verbose(
                    f"CascadeCorrelationNetwork: _collect_training_results: Collected {collected_results}/{num_tasks}"
                )
            except Empty as empty_e:
                self.logger.warning(
                    f"CascadeCorrelationNetwork: _collect_training_results: Result queue empty, continuing: {empty_e}"
                )
                continue
            except Exception as e:
                self.logger.error(
                    f"CascadeCorrelationNetwork: _collect_training_results: Error collecting result: {e}"
                )
                import traceback
                ...
                break
        self.logger.debug(
            f"CascadeCorrelationNetwork: _collect_training_results: Collected {collected_results} results"
        )
        return results
    ...
    def _stop_workers(self, workers: list, task_queue) -> None:
        ...
        if not workers:
            self.logger.debug(
                "CascadeCorrelationNetwork: _stop_workers: No workers to stop"
            )
            return
        self.logger.info(
            f"CascadeCorrelationNetwork: _stop_workers: Stopping {len(workers)} worker processes"
        )
        ...
        for i in range(len(workers)):
            try:
                ...
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _stop_workers: Sent sentinel to worker {i}"
                )
            except Exception as e:
                self.logger.error(
                    f"CascadeCorrelationNetwork: _stop_workers: Failed to send sentinel to worker {i}: {e}"
                )
        ...
        for worker in workers:
            ...
            if not worker.is_alive():
                terminated_count += 1
                self.logger.debug(
                    f"CascadeCorrelationNetwork: _stop_workers: Worker {worker.name} stopped gracefully"
                )
            else:
                self.logger.warning(
                    f"CascadeCorrelationNetwork: _stop_workers: Worker {worker.name} (PID {worker.pid}) did not stop gracefully"
                )
        ...
        for worker in workers:
            if worker.is_alive():
                self.logger.warning(
                    f"CascadeCorrelationNetwork: _stop_workers: Terminating worker {worker.name}"
                )
                worker.terminate()
                ...
                if worker.is_alive():
                    self.logger.error(
                        f"CascadeCorrelationNetwork: _stop_workers: Worker {worker.name} still alive, sending SIGKILL"
                    )
                    try:
                        ...
                        self.logger.error(
                            f"CascadeCorrelationNetwork: _stop_workers: Failed to SIGKILL worker: {e}"
                        )

        if alive_workers := [w for w in workers if w.is_alive()]:
            self.logger.error(
                f"CascadeCorrelationNetwork: _stop_workers: ⚠️  {len(alive_workers)} workers still alive after cleanup!"
            )
        else:
            self.logger.info(
                f"CascadeCorrelationNetwork: _stop_workers: ✓ All {len(workers)} workers stopped successfully ({terminated_count} gracefully)"
            )

    def _process_training_results(
        self, results: list, tasks: list, start_time
    ) -> TrainingResults:
        """
        ...
        self.logger.info(
            f"CascadeCorrelationNetwork: _process_training_results: Training duration: {end_time - start_time}"
        )

        # Process results
        if not results:
            self.logger.warning(
                "CascadeCorrelationNetwork: _process_training_results: No results obtained"
            )
            self.logger.warning(
                f"CascadeCorrelationNetwork: _process_training_results: Unable to Process empty results list.  Building dummy results: {results}"
            )
            results = self._get_dummy_results(len(tasks))
        elif len(results) != len(tasks):
            self.logger.warning(
                f"CascadeCorrelationNetwork: _process_training_results: Mismatch in results count: expected {len(tasks)}, got {len(results)}"
            )
        self.logger.trace(
            "CascadeCorrelationNetwork: _process_training_results: Completed pre-processing of training results."
        )
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _process_training_results: Sorted {len(results)} results"
        )
        ...
        # Compile statistics
        successful_candidates = self.get_candidates_data_count(
            results, "correlation", lambda c: c >= self.correlation_threshold
        )
        success_count = self.get_candidates_data_count(results, "success", lambda s: s)
        if success_count != successful_candidates:
            self.logger.warning(
                f"CascadeCorrelationNetwork: _process_training_results: Mismatch in success counts: success_count: {success_count}, successful_candidates: {successful_candidates}"
            )
        ...
        training_results = TrainingResults(
            ...
            best_candidate_uuid=(
                getattr(best_result, "candidate_uuid", None) if best_result else None
            ),
            best_correlation=(
                getattr(best_result, "correlation", 0.0) if best_result else 0.0
            ),
            best_candidate=(
                getattr(best_result, "candidate", None) if best_result else None
            ),
            success_count=success_count,
            successful_candidates=successful_candidates,
            failed_count=len(results) - successful_candidates,
            error_messages=self.get_candidates_error_messages(
        ...
        self.logger.debug(
            f"CascadeCorrelationNetwork: _process_training_results: Processed results: {training_results}"
        )
        self.logger.trace(
            "CascadeCorrelationNetwork: _process_training_results: Completed processing of training results."
        )
    ...
    def get_single_candidate_data(
        ...
        """
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: Retrieving field '{field}' for candidate ID {candidate_id}"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: Results type: {type(results)}, length: {len(results)}, Results: {results}"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: Field: {field}, Default: {default}"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: ID: type: {type(candidate_id)}, value: {candidate_id}"
        )
        ...
        candidate_id = (
            ...
            else candidate_id if isinstance(candidate_id, int) else None
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: Processed Candidate ID: type: {type(candidate_id)}, value: {candidate_id}"
        )
        if candidate_id is not None and 0 <= candidate_id < len(results):
            value = getattr(results[candidate_id], field, None)
            self.logger.debug(
                f"CascadeCorrelationNetwork: get_single_candidate_data: Retrieved value: {value}"
            )
            return value if value is not None else default
        self.logger.debug(
            f"CascadeCorrelationNetwork: get_single_candidate_data: ID {candidate_id} is out of bounds, returning default: {default}"
        )
        return default
    ...
    def get_candidates_data_count(
        ...
        return sum(
            getattr(r, field)
            for r in results
            if getattr(r, field) is not None and constraint(getattr(r, field))
        )

    def get_candidates_error_messages(
        self, results: list, valid_candidates: list
    ) -> dict:
        """
        ...
        return {
            key: (
                f'Candidate ID {r.candidate_id} (UUID: {r.candidate_uuid}): "{r.error_message}"'
                if r.error_message and valid_candidates[i]
                else (
                    f"Candidate ID {r.candidate_id} (UUID: {r.candidate_uuid}): No error message provided"
    ...
    def train_candidate_worker(
        ...
        try:
            ...
            result = CascadeCorrelationNetwork._train_candidate_unit(
                ...
                candidate_display_frequency=candidate_inputs.get(
                    "candidate_display_frequency"
                ),
                worker_id=worker_id,
                worker_uuid=worker_uuid,
    ...
    def _start_manager(self):
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _start_manager: Starting multiprocessing manager"
        )
        if self._manager is not None:
            self.logger.warning(
                "CascadeCorrelationNetwork: _start_manager: Manager already started"
            )
            return
        ...
        try:
            self._manager = CandidateTrainingManager(
                address=address, authkey=authkey, ctx=self._mp_ctx
            )
            ...
            self.logger.info(
                f"CascadeCorrelationNetwork: _start_manager: Manager started at {address}"
            )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: _start_manager: Failed to start manager: {e}"
            )
            raise
    ...
    def _stop_manager(self):
        ...
        self.logger.debug(
            "CascadeCorrelationNetwork: _stop_manager: Stopping multiprocessing manager"
        )
        if self._manager is not None:
            try:
                self._manager.shutdown()
                self.logger.info(
                    "CascadeCorrelationNetwork: _stop_manager: Manager shutdown completed"
                )
            except Exception as e:
                self.logger.error(
                    f"CascadeCorrelationNetwork: _stop_manager: Error shutting down manager: {e}"
                )
            finally:
                self._manager = None
                self._task_queue = None
    ...
    def _worker_loop(
        ...
        while True:
            try:
                ...
                logger.debug(
                    "CascadeCorrelationNetwork: _worker_loop: No task available, entering stand-by mode"
                )
                time.sleep(0.1)
                continue
            except Exception as e:
                logger.critical(
                    f"CascadeCorrelationNetwork: _worker_loop: Worker critical get error: {e}"
                )
                import traceback

                logger.critical(
                    f"CascadeCorrelationNetwork: _worker_loop: Traceback: {traceback.format_exc()}"
                )
                break
            ...
            if task is None:
                logger.debug(
                    "CascadeCorrelationNetwork: _worker_loop: Received sentinel, stopping worker"
                )
                break
            try:
                ...
                # Process the task
                logger.debug(
                    f"CascadeCorrelationNetwork: _worker_loop: Processing task: {task[0] if task else 'None'}"
                )
                result = CascadeCorrelationNetwork.train_candidate_worker(
                    task_data_input=task, parallel=parallel
                )
                logger.debug(
                    "CascadeCorrelationNetwork: _worker_loop: Task processed, putting result in queue"
                )
                ...
                try:
                    ...
                    logger.debug(
                        "CascadeCorrelationNetwork: _worker_loop: Task completed successfully"
                    )
                except Full as fe:
                    logger.error(
                        f"CascadeCorrelationNetwork: _worker_loop: Result queue full, dropping result: {fe}"
                    )
                    import traceback
                ...
                logger.error(
                    f"CascadeCorrelationNetwork: _worker_loop: Worker task error: {e}"
                )
                import traceback
                ...
                try:
                    ...
                    candidate_uuid = (
                        task[1][4]
                        if task and len(task) > 1 and len(task[1]) > 4
                        else None
                    )
                    ...
                    logger.debug(
                        "CascadeCorrelationNetwork: _worker_loop: Put failure result"
                    )
                except Full as fq_e:
                    logger.error(
                        f"CascadeCorrelationNetwork: _worker_loop: Failed to put failure result - queue full: {fq_e}"
                    )
                    import traceback
                    ...
                    logger.error(
                        f"CascadeCorrelationNetwork: _worker_loop: Failed to put failure result: {put_e}"
                    )
                    import traceback
    ...
    def calculate_residual_error(
        ...
        """
        self.logger.debug(
            f"CascadeCorrelationNetwork: calculate_residual_error: Calculating residual error for input shape: {x.shape if isinstance(x, torch.Tensor) else 'None'}, target shape: {y.shape if isinstance(y, torch.Tensor) else 'None'}"
        )
        x = (x, torch.empty(0, self.input_size))[x is None]
        self.logger.debug(
            f"CascadeCorrelationNetwork: calculate_residual_error: After defaulting, input shape: {x.shape if isinstance(x, torch.Tensor) else 'None'}, target shape: {y.shape if isinstance(y, torch.Tensor) else 'None'}"
        )
        ...
        if not isinstance(x, torch.Tensor) or not isinstance(y, torch.Tensor):
            ...
            self.logger.debug(
                f"CascadeCorrelationNetwork: calculate_residual_error: Input and target must be torch.Tensor, x type: {type(x)}, y type: {type(y)}"
            )
            x = torch.empty(0, self.input_size)
            y = torch.empty(0, self.output_size)
            self.logger.debug(
                f"CascadeCorrelationNetwork: calculate_residual_error: After defaulting, input shape: {x.shape}, target shape: {y.shape}"
            )
        # Check batch size match (x and y must have same number of samples)
        if x.shape[0] != y.shape[0]:
            self.logger.debug(
                f"CascadeCorrelationNetwork: calculate_residual_error: Input and target must have the same batch size (dim 0), x shape: {x.shape}, y shape: {y.shape}"
            )
            # Return empty residual for mismatched batch sizes
        elif y.shape[1] != self.output_size:
            self.logger.debug(
                f"CascadeCorrelationNetwork: calculate_residual_error: Target must have same output size as network, expected {self.output_size}, got {y.shape[1]}"
            )
            # Return empty residual for mismatched output size
        else:
            # result = torch.empty(0, simple_network.input_size)
            self.logger.debug(
                "CascadeCorrelationNetwork: calculate_residual_error: Forward pass to calculate output for residual error computation"
            )
            with torch.no_grad():
                self.logger.debug(
                    "CascadeCorrelationNetwork: calculate_residual_error: Performing forward pass without gradient tracking"
                )
                output = self.forward(x)
                self.logger.debug(
                    f"CascadeCorrelationNetwork: calculate_residual_error: Forward pass completed, output shape: {output.shape}, Output:\n{output}"
                )
                residual = y - output
                self.logger.debug(
                    f"CascadeCorrelationNetwork: calculate_residual_error: Calculated residual error, shape: {residual.shape}, Residual Error:\n{residual}"
                )
            self.logger.verbose(
                f"CascadeCorrelationNetwork: calculate_residual_error: Validating residual error, shape: {residual.shape}, Residual Error:\n{residual}"
            )
        ...
        self.logger.verbose(
            f"CascadeCorrelationNetwork: calculate_residual_error: Returning residual error, shape: {residual.shape}, Residual Error:\n{residual}"
        )
        return residual
    ...
    def _select_best_candidates(self, results: list, num_candidates: int = 1) -> list:
        ...
        """
        self.logger.debug(
            f"CascadeCorrelationNetwork: _select_best_candidates: Selecting top {num_candidates} from {len(results)} candidates"
        )
        ...
        self.logger.info(
            f"CascadeCorrelationNetwork: _select_best_candidates: Selected {len(selected)} candidates with correlations: {[r.correlation for r in selected]}"
        )
        return selected
    ...
    def add_units_as_layer(self, candidates: list, x: torch.Tensor) -> None:
        ...
        """
        self.logger.info(
            f"CascadeCorrelationNetwork: add_units_as_layer: Adding layer with {len(candidates)} units"
        )
        for candidate in candidates:
            if candidate.candidate and hasattr(candidate.candidate, "weights"):
                ...
                self.logger.warning(
                    f"CascadeCorrelationNetwork: add_units_as_layer: Skipping invalid candidate: {candidate}"
                )
        self.logger.info(
            f"CascadeCorrelationNetwork: add_units_as_layer: Layer added, total hidden units: {len(self.hidden_units)}"
        )
    ...
    def grow_network(
        ...
        for epoch in range(max_epochs):
            ...
            residual_error = self._calculate_residual_error_safe(
                x_train=x_train, y_train=y_train
            )
            if residual_error is None:
                self.logger.warning(
                    "CascadeCorrelationNetwork: grow_network: Residual error is None, stopping growth of the network."
                )
                break
            self.logger.debug(
            ...
            if (
                not (
                    training_results := self._get_training_results(
                        x_train=x_train, y_train=y_train, residual_error=residual_error
                    )
                )
                or not training_results.best_candidate
            ):
                self.logger.warning(
                    "CascadeCorrelationNetwork: grow_network: Training results are None or best candidate is None, stopping growth of the network."
                )
                break

            # Check if best candidate meets correlation threshold
            elif (
                training_results.best_candidate.get_correlation()
                < self.correlation_threshold
            ):
                self.logger.info(
                    f"CascadeCorrelationNetwork: grow_network: No candidate met correlation threshold: {self.correlation_threshold}, Best Correlation Achieved: {training_results.best_candidate.get_correlation():.6f}"
                )
                break
            self.logger.info(
                f"CascadeCorrelationNetwork: grow_network: Best Candidate: {training_results.best_candidate.get_correlation() if training_results.best_candidate else None}, Met correlation threshold: {self.correlation_threshold}"
            )
            ...
            if candidates_per_layer > 1:
                if selected_candidates := self._select_best_candidates(
                    ...
                    self.add_units_as_layer(
                        [c for c in selected_candidates if c.candidate], x_train
                    )
                    train_loss = self.train_output_layer(
                        x_train, y_train, self.output_epochs
                    )
                    train_accuracy = self.get_accuracy(x_train, y_train)
                    self.logger.info(
                        f"CascadeCorrelationNetwork: grow_network: Added {len(selected_candidates)} candidates as layer"
                    )
                else:
                    self.logger.warning(
                        "CascadeCorrelationNetwork: grow_network: No candidates met selection criteria"
                    )
                    break
            else:

                # Original behavior: Add single best candidate
                train_loss, train_accuracy = self._add_best_candidate(
                    training_results.best_candidate, x_train, y_train, epoch
                )
            self.logger.debug(
                f"CascadeCorrelationNetwork: grow_network: After adding candidate(s), Training Loss: {train_loss:.6f}, Training Accuracy: {train_accuracy:.4f}, For Current Epoch {epoch}, Post-Train History:\n{self.history}"
            )
            ...
            self.logger.debug(
                f"CascadeCorrelationNetwork: grow_network: Validate Training Inputs: {validate_training_inputs}"
            )

            # Validation of training results
            try:
                validate_training_results: ValidateTrainingResults = (
                    self.validate_training(validate_training_inputs)
                )
                self.logger.debug(
                    f"CascadeCorrelationNetwork: grow_network: Validation Results: {validate_training_results}"
                )
            except Exception as e:
                self.logger.error(
                    f"CascadeCorrelationNetwork: grow_network: Caught Exception while validating training at epoch {epoch + 1}/{max_epochs}:\nException:\n{e}"
                )
                import traceback
            ...
            if validate_training_results.early_stop:
                ...
                break
            self.logger.info(
                f"CascadeCorrelationNetwork: grow_network: Epoch {epoch} - Train Loss: {train_loss:.6f}, Train Accuracy: {train_accuracy:.4f}, Early stop: {validate_training_results.early_stop}"
            )
        if not validate_training_results:
            self.logger.warning(
                f"CascadeCorrelationNetwork: grow_network: Validation failed at epoch {epoch + 1}/{max_epochs}."
            )
            validate_training_results = ValidateTrainingResults(
                early_stop=False,
                patience_counter=patience_counter,
    ...
    def _calculate_residual_error_safe(
        ...
        if (
            ...
            or y_train.shape[0] == 0
        ):
            self.logger.warning(
                "CascadeCorrelationNetwork: _calculate_residual_error_safe: Training data is None or empty, cannot calculate residual error"
            )
            return None
        try:
            self.logger.debug(
                f"CascadeCorrelationNetwork: _calculate_residual_error_safe: Starting epoch {epoch + 1}/{max_epochs}"
            )
            residual_error = self.calculate_residual_error(x_train, y_train)
            self.logger.debug(
                f"CascadeCorrelationNetwork: _calculate_residual_error_safe: Epoch {epoch}, Residual Error: {residual_error.mean().item():.6f}"
            )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: _calculate_residual_error_safe: Caught Exception while logging epoch {epoch + 1}/{max_epochs}:\nException:\n{e}"
            )
            import traceback
    ...
    def _get_training_results(
        ...
        try:
            ...
            self.logger.debug(
                f"CascadeCorrelationNetwork: _get_training_results: Training Results - Epoch {epoch}, Successful: {training_results.successful_candidates}, Failed: {training_results.failed_count}"
            )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: _get_training_results: Caught Exception while training candidates at epoch {epoch + 1}/{max_epochs}:\nException:\n{e}"
            )
            import traceback
    ...
    def _add_best_candidate(
        ...
        self.logger.info(
            f"CascadeCorrelationNetwork: _add_best_candidate: Adding best candidate {best_candidate} at epoch {epoch}"
        )
        if best_candidate is None:
            self.logger.warning(
                "CascadeCorrelationNetwork: _add_best_candidate: Best candidate is None, cannot add to network"
            )
            return None, None
        try:
            ...
            self.logger.info(
                "CascadeCorrelationNetwork: _add_best_candidate: Added best candidate to the network"
            )
            train_loss = self._retrain_output_layer(
                x_train=x_train, y_train=y_train, epochs=self.output_epochs, epoch=epoch
            )
            self.logger.debug(
                f"CascadeCorrelationNetwork: _add_best_candidate: Training Loss: {train_loss}, For Current Epoch {epoch}, Post-Train Loss History:\n{self.history}"
            )
            train_accuracy = self._calculate_train_accuracy(
                x_train=x_train, y_train=y_train, epoch=epoch
            )
            self.logger.debug(
                f"CascadeCorrelationNetwork: _add_best_candidate: Training Accuracy: {train_accuracy}, For Current Epoch {epoch}, Post-Train Accuracy History:\n{self.history}"
            )
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: _add_best_candidate: Caught Exception while adding unit and retraining output layer at epoch {epoch + 1}/{max_epochs}:\nException:\n{e}"
            )
            import traceback
    ...
    def _calculate_train_accuracy(
        self, x_train: torch.Tensor = None, y_train: torch.Tensor = None, epoch: int = 0
    ) -> float:
        # Validate method input parameters
        if (
            x_train is None
            ...
            self.logger.warning(
                "CascadeCorrelationNetwork: _calculate_train_accuracy: Training data is None or empty, cannot calculate accuracy"
            )
            return 0.0
        if x_train.shape[0] != y_train.shape[0]:
            self.logger.warning(
                f"CascadeCorrelationNetwork: _calculate_train_accuracy: Training data and target have different number of samples, x_train shape: {x_train.shape}, y_train shape: {y_train.shape}, cannot calculate accuracy"
            )
            return 0.0
        ...
        train_accuracy = self.calculate_accuracy(x_train, y_train)
        self.logger.debug(
            f"CascadeCorrelationNetwork: _calculate_train_accuracy: For Current Epoch {epoch}, Train Accuracy: {train_accuracy:.4f}"
        )

        # Update training history
        self.history["train_accuracy"].append(train_accuracy)
        self.logger.debug(
            f"CascadeCorrelationNetwork: _calculate_train_accuracy: For Current Epoch {epoch}, Post-Train Accuracy History:\n{self.history}"
        )
    ...
    def _retrain_output_layer(
        ...
        if (
            ...
            self.logger.warning(
                "CascadeCorrelationNetwork: _retrain_output_layer: Training data is None or empty, cannot retrain output layer"
            )
            return float("inf")
        if x_train.shape[0] != y_train.shape[0]:
            self.logger.warning(
                f"CascadeCorrelationNetwork: _retrain_output_layer: Training data and target have different number of samples, x_train shape: {x_train.shape}, y_train shape: {y_train.shape}, cannot retrain output layer"
            )
            return float("inf")
        if epochs <= 0:
            self.logger.warning(
                f"CascadeCorrelationNetwork: _retrain_output_layer: Number of epochs for retraining output layer is non-positive: {epochs}, skipping retraining"
            )
            return float("inf")
        self.logger.info(
            f"CascadeCorrelationNetwork: _retrain_output_layer: Retraining output layer for {epochs} epochs after adding new hidden unit"
        )

        # Retrain output layer
        train_loss = self.train_output_layer(x_train, y_train, self.output_epochs)
        self.logger.info(
            f"CascadeCorrelationNetwork: _retrain_output_layer: Full Network Training Loss after Epoch {epoch}, Train Loss: {train_loss:.6f}"
        )
        self.logger.debug(
            f"CascadeCorrelationNetwork: _retrain_output_layer: For Current Epoch: {epoch}, Post-Train Loss History:\n{self.history}"
        )

        # Update training history
        self.history["train_loss"].append(train_loss)
        self.logger.debug(
            f"CascadeCorrelationNetwork: _retrain_output_layer: For Current Epoch: {epoch}, Post-Trained History:\n{self.history}"
        )
        return train_loss
    ...
    def create_snapshot(
        self, snapshot_dir: Union[str, pl.Path] = None
    ) -> Optional[pl.Path]:
        """
        ...
        try:
            ...
            if snapshot_dir is None:
                snapshot_dir = pl.Path(
                    self.cascade_correlation_network_snapshots_dir
                ) or pl.Path(_CASCADE_CORRELATION_NETWORK_HDF5_PROJECT_SNAPSHOTS_DIR)
            else:
            ...
            if self._save_to_hdf5(
                ...
                self.logger.info(
                    f"CascadeCorrelationNetwork: create_snapshot: Created snapshot at {snapshot_path}"
                )
    ...
    def restore_snapshot(
        ...
        try:
            if snapshot_path is None:
                logger.error(
                    "CascadeCorrelationNetwork: restore_snapshot: No snapshot path provided"
                )
                return False
            snapshot_path = pl.Path(snapshot_path)
            if not snapshot_path.exists():
                logger.error(
                    f"CascadeCorrelationNetwork: restore_snapshot: Snapshot file does not exist: {snapshot_path}"
                )
                return False
            ...
            if loaded_network is None:
                logger.error(
                    f"CascadeCorrelationNetwork: restore_snapshot: Failed to load network from snapshot: {snapshot_path}"
                )
                return False
            ...
            logger.info(
                f"CascadeCorrelationNetwork: restore_snapshot: Restored snapshot from {snapshot_path}"
            )
            return True
        except Exception as e:
            logger.error(
                f"CascadeCorrelationNetwork: restore_snapshot: Error restoring snapshot: {e}"
            )
            import traceback
    ...
    def save_object(
        self, objectify: Any = None, snapshot_dir: Union[str, pl.Path] = None
    ) -> Optional[pl.Path]:
        """
    ...
    def _save_object_hdf5(
        ...
        try:
            ...
            if create_backup and os.path.exists(filepath):
                ...
                self.logger.info(
                    f"CascadeCorrelationNetwork: Created backup at {backup_path}"
                )
            ...
            if success := serializer.save_object(
                ...
                self.logger.info(
                    f"CascadeCorrelationNetwork: save_to_hdf5: Successfully saved to {filepath}"
                )
            else:
                self.logger.error(
                    f"CascadeCorrelationNetwork: save_to_hdf5: Failed to save to {filepath}"
                )
            self.logger.debug(
                "CascadeCorrelationNetwork: save_to_hdf5: Verifying saved HDF5 file"
            )
            checked_object = self.verify_hdf5_file(filepath)
            if not checked_object.get("valid", False):
                self.logger.error(
                    f"CascadeCorrelationNetwork: save_to_hdf5: Verification failed for saved HDF5 file: {filepath}, Error: {checked_object.get('error', 'Unknown error')}"
                )
                return False
            self.logger.info(
                f"CascadeCorrelationNetwork: save_to_hdf5: Verified saved HDF5 file is valid: {filepath}"
            )
            return success
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: save_to_hdf5: Error saving to HDF5: {e}"
            )
            import traceback
    ...
    def _save_to_hdf5(
        ...
        try:
            ...
            if not checked_network.get("valid", False):
                self.logger.error(
                    f"CascadeCorrelationNetwork: save_to_hdf5: Verification failed for saved HDF5 file: {filepath}, Error: {checked_network.get('error', 'Unknown error')}"
                )
                return False
            self.logger.info(
                f"CascadeCorrelationNetwork: save_to_hdf5: Verified saved HDF5 file is valid: {filepath}"
    ...
    @classmethod
    def load_from_hdf5(
        cls, filepath: Union[str, pl.Path], restore_multiprocessing: bool = False
    ):
        """
        ...
        """
        return cls._load_from_hdf5(
            filepath=filepath, restore_multiprocessing=restore_multiprocessing
        )
    ...
    def _load_from_hdf5(
        ...
        try:
            ...
            network = serializer.load_network(
                filepath=filepath, restore_multiprocessing=restore_multiprocessing
            )
            if network:
                network.logger.info(
                    f"CascadeCorrelationNetwork: load_from_hdf5: Successfully loaded from {filepath}"
                )
            else:
                logger.error(
                    f"CascadeCorrelationNetwork: load_from_hdf5: Failed to load from {filepath}"
                )
            return network
        except Exception as e:
            logger.error(
                f"CascadeCorrelationNetwork: load_from_hdf5: Error loading from HDF5: {e}"
            )
            import traceback
    ...
    def list_hdf5_snapshots(self, directory: Union[str, pl.Path]) -> List[pl.Path]:
        ...
        try:
            ...
            if not directory.exists() or not directory.is_dir():
                self.logger.error(
                    f"CascadeCorrelationNetwork: list_hdf5_snapshots: Directory does not exist: {directory}"
                )
                return []
            hdf5_files = HDF5Utils.list_hdf5_files(directory)
            self.logger.info(
                f"CascadeCorrelationNetwork: list_hdf5_snapshots: Found {len(hdf5_files)} HDF5 files in {directory}"
            )
            return hdf5_files
        except Exception as e:
            self.logger.error(
                f"CascadeCorrelationNetwork: list_hdf5_snapshots: Error listing HDF5 files: {e}"
            )
            import traceback
    ...
    def verify_hdf5_file(self, filepath: Union[str, pl.Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"CascadeCorrelationNetwork: Error verifying HDF5 file: {e}"
            )
            import traceback
    ...
    def predict(
        self, x: torch.Tensor
    ) -> torch.Tensor:  # sourcery skip: class-extract-method
        """
