"""Build-provenance accessors for juniper-cascor.

The image's ``Dockerfile`` stamps the source git SHA and an ISO-8601 build
timestamp into the ``JUNIPER_CASCOR_GIT_SHA`` / ``JUNIPER_CASCOR_BUILD_DATE``
...
``/v1/health`` and the Prometheus ``juniper_cascor_build`` Info metric can
...
def git_sha() -> str | None:
    ...
    Read from ``JUNIPER_CASCOR_GIT_SHA`` (set by the Dockerfile from a
    build-arg). ``None`` outside a provenance-stamped image.
    """
    return os.environ.get("JUNIPER_CASCOR_GIT_SHA") or None


def build_date() -> str | None:
    """Return the ISO-8601 image build timestamp, or ``None``.

    Read from ``JUNIPER_CASCOR_BUILD_DATE`` (set by the Dockerfile from a
    build-arg). ``None`` outside a provenance-stamped image.
    """
    return os.environ.get("JUNIPER_CASCOR_BUILD_DATE") or None
