...
class TrainingParamUpdateRequest(BaseModel):
    ...
    convergence_threshold: Optional[float] = Field(None, gt=0, le=1.0, description="Minimum loss improvement to reset patience counter")
    candidate_patience: Optional[int] = Field(None, ge=1, description="Candidate training early stopping patience epochs")
    candidate_convergence_threshold: Optional[float] = Field(None, gt=0, le=1.0, description="Minimum correlation improvement to reset candidate patience counter")
