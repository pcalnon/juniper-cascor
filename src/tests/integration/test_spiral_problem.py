...
class TestSpiralProblemBasic:
    ...
    def test_2_spiral_learning(self, spiral_network):
        ...
        # Generate 2-spiral data - smaller dataset in fast mode
        n_per_spiral = 20 if fast_mode else 50
        ...
        # Train network - fewer epochs in fast mode
        max_epochs = 3 if fast_mode else 10
        initial_accuracy = spiral_network.calculate_accuracy(x, y)
        history, elapsed_time = measure_training_time(
            spiral_network, x, y, max_epochs=max_epochs, early_stopping=False
        )
        final_accuracy = spiral_network.calculate_accuracy(x, y)

        # In fast mode, just verify the network doesn't crash and produces valid output
        # In normal mode, verify learning occurred
        if not fast_mode:
            ...
            # Fast mode: just verify accuracy is valid (between 0 and 1)
    ...
    @pytest.mark.slow
    @pytest.mark.timeout(60 if _is_fast_mode() else 300)
    def test_3_spiral_learning(self, spiral_network):
        """Test that network can learn 3-spiral problem."""
        set_deterministic_behavior(42)
        fast_mode = _is_fast_mode()

        # Generate 3-spiral data - smaller dataset in fast mode
        n_per_spiral = 15 if fast_mode else 40
        ...
        # Train network - fewer epochs in fast mode
        max_epochs = 3 if fast_mode else 8
        initial_accuracy = spiral_network.calculate_accuracy(x, y)
...
class TestSpiralProblemProgressive:
    ...
    def test_n_spiral_difficulty_progression(self, n_spirals):
        ...
        network = CascadeCorrelationNetwork.create_simple_network(
            ...
            max_hidden_units=2 if fast_mode else min(8, n_spirals * 2),
            candidate_pool_size=2 if fast_mode else 12,
            correlation_threshold=0.02 if fast_mode else 0.15
        )

        # Generate n-spiral data - smaller in fast mode
        n_per_spiral = 10 if fast_mode else 30
        x, y, _ = SpiralDataGenerator.generate_n_spiral(
            n_spirals=n_spirals,
            n_per_spiral=n_per_spiral,
            noise=0.02,
        ...
        # Train network - fewer epochs in fast mode
        max_epochs = 2 if fast_mode else 6
        ...
        # In fast mode, just verify valid output
        if not fast_mode:
...
class TestSpiralProblemRobustness:
    ...
    def test_spiral_noise_robustness(self, spiral_network, noise_level):
        ...
        # Generate noisy spiral data - smaller in fast mode
        n_per_spiral = 15 if fast_mode else 40
        x, y, _ = SpiralDataGenerator.generate_2_spiral(
            n_per_spiral=n_per_spiral,
            noise=noise_level,
    ...
    def test_spiral_data_size_scaling(self, spiral_network, n_per_spiral):
        ...
        # In fast mode, reduce data sizes proportionally
        actual_n_per_spiral = max(10, n_per_spiral // 5) if fast_mode else n_per_spiral
        
        # Generate spiral data of different sizes
        x, y, _ = SpiralDataGenerator.generate_2_spiral(
            n_per_spiral=actual_n_per_spiral,
            noise=0.03,
            seed=42
        )

        # Train network - fewer epochs in fast mode
        max_epochs = 2 if fast_mode else 5
        initial_accuracy = spiral_network.calculate_accuracy(x, y)
        history = spiral_network.fit(x, y, max_epochs=max_epochs)
        final_accuracy = spiral_network.calculate_accuracy(x, y)
...
class TestSpiralProblemVisualization:
    ...
    def test_spiral_training_progression(self, spiral_network):
        ...
        # Generate spiral data - smaller in fast mode
        n_per_spiral = 20 if fast_mode else 60
        x, y, _ = SpiralDataGenerator.generate_2_spiral(
            n_per_spiral=n_per_spiral,
            noise=0.04,
            seed=42
        ...
        # Train with validation - fewer epochs in fast mode
        max_epochs = 3 if fast_mode else 8
...
class TestSpiralProblemEdgeCases:
    ...
    def test_perfect_spiral_separation(self, spiral_network):
        ...
        # Generate data with no noise (perfect separation) - smaller in fast mode
        n_per_spiral = 10 if fast_mode else 30
        x, y, _ = SpiralDataGenerator.generate_2_spiral(
            n_per_spiral=n_per_spiral,
...
class TestSpiralProblemComparison:
    ...
    def test_spiral_configuration_comparison(self):
        ...
        # Reduce configurations in fast mode
        if fast_mode:
            configurations = [
        ...
        for config in configurations:  # sourcery skip: no-loop-in-tests
            # Create fresh network for each test - smaller in fast mode
            from cascade_correlation.cascade_correlation import CascadeCorrelationNetwork
            network = CascadeCorrelationNetwork.create_simple_network(
                input_size=2,
                output_size=config["n_spirals"],
                learning_rate=0.1 if fast_mode else 0.04,
                max_hidden_units=2 if fast_mode else 6,
                candidate_pool_size=2 if fast_mode else 8,
                correlation_threshold=0.02 if fast_mode else 0.1
            )
            ...
            # Train and measure - fewer epochs in fast mode
            max_epochs = 2 if fast_mode else 6
            initial_accuracy = network.calculate_accuracy(x, y)
            ...
            # In fast mode, just validate structure
            if not fast_mode:
                # Basic validation
                assert final_accuracy >= initial_accuracy  # trunk-ignore(bandit/B101)
            assert_network_structure_valid(network)
        ...
        # All configurations should achieve some learning (only in normal mode)
        if not fast_mode:
            for result in results:  # sourcery skip: no-loop-in-tests
