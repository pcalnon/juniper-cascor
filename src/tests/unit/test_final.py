...
def test_candidate_units_simple(fast_training_params):
    ...
    for i in range(3):  # sourcery skip: no-loop-in-tests
        ...
        candidate = CandidateUnit(CandidateUnit__input_size=2, CandidateUnit__candidate_index=i, CandidateUnit__epochs=test_epochs, CandidateUnit__learning_rate=0.01, CandidateUnit__log_level_name="ERROR", CandidateUnit__display_frequency=10000)  # Minimal logging  # No display to avoid errors

        print(f"  Initial weights: {candidate.weights.detach().numpy()}")
        print(f"  Initial bias: {candidate.bias.item():.4f}")

        # Train the candidate
        final_correlation = candidate.train(x=x, epochs=test_epochs, residual_error=residual_error, learning_rate=0.01, display_frequency=10000)  # No display to avoid errors
