"""
Pin the CPU-only torch contract between the Dockerfile, requirements-cpu.lock, the publish
workflow and the in-image check script.

Regression guard for the class the worker's first published image fell into (2026-09-07:
``torch 2.12.1+cu130`` plus the ``nvidia-*`` / ``triton`` stack, 3 GB per Raspberry Pi node) and
for this repo's own variant of it: ``requirements.lock`` is resolved against the CUDA torch on
PyPI, so it pins ``nvidia-cublas`` / ``nvidia-cudnn-cu13`` / ``triton`` / ``cuda-toolkit``
OUTRIGHT, and a Dockerfile that installs it wholesale ships ~3 GB of CUDA libraries on a CPU-only
image whatever torch build it carries. The image therefore installs ``requirements-cpu.lock``
(that lock minus the CUDA stack) plus torch pinned to ``ARG TORCH_VERSION``+cpu in BOTH installs,
with the CPU index as an extra index on the lock install.
...
class TestDockerfileTorchPin:
    ...
    def test_cpu_lock_excludes_torch_and_the_cuda_stack(self):
        """The whole point of the second lock: no torch pin (it would be PyPI's) and nothing from the CUDA stack."""
        offenders = [p for p in _pins(CPU_LOCK) if p.startswith("torch==") or p.startswith(CUDA_STACK_PREFIXES)]
        assert offenders == [], f"requirements-cpu.lock must not pin torch or any CUDA-stack package: {offenders}"
    ...
    def test_dockerfile_installs_the_cpu_lock_not_the_gpu_lock(self):
        text = _dockerfile_text()
        assert "COPY requirements-cpu.lock" in text
        assert "-r requirements-cpu.lock" in text
        assert "-r requirements.lock" not in text, "the image must never install requirements.lock -- it pins the CUDA stack outright"
...
class TestPublishWorkflowAssertsContract:
    def test_paths_filter_covers_the_image_inputs_and_the_check(self):
        ...
        for required in ("Dockerfile", "requirements-cpu.lock", "pyproject.toml", "src/**", CHECK_SCRIPT_REL, ".github/workflows/publish-image.yml"):
    ...
    def test_jobs_are_guarded_to_this_packages_release_tags(self):
        """REQUIRED here: protocol / model releases fire the same event and must not republish `latest`."""
        jobs = _workflow()["jobs"]
    ...
    def test_smoke_test_runs_the_check_and_imports_the_service(self):
        run = _step("build", "Smoke test")["run"]
        assert CHECK_SCRIPT_REL in run
        assert "EXPECT_TORCH=" in run
        assert "import cascade_correlation, api, snapshots" in run
...
# ─────────────────────────────────────────────────────────────────────────────────────────────
# CI keeps the CPU lock complete
# ─────────────────────────────────────────────────────────────────────────────────────────────
class TestCiGuardsTheCpuLock:
    def test_lockfile_freshness_job_checks_the_cpu_lock(self):
        ci = yaml.safe_load((REPO / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8"))
        runs = "\n".join(str(s.get("run", "")) for s in ci["jobs"]["lockfile-check"]["steps"])
        assert "requirements-cpu.lock" in runs
        for extra in ("ml", "api", "observability", "juniper-data"):
            assert f'"{extra}"' in runs, f"the CPU-lock completeness check must cover the {extra!r} extra the lock is compiled with"
