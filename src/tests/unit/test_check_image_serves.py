...
:8200, started as deployed) and that every version it reports is the one it was built as. The
published ``juniper-cascor:0.11.0`` is why: it imports, answers ``/v1/health`` with ``0.11.0``, and
stamps ``meta.version: "0.6.0"`` on every enveloped response (juniper-cascor#668, fixed by #672).
Every publish-path check passed it. The image ships no ``juniper_cascor`` package -- the Dockerfile
copies ``src/`` -- so there is no ``__version__`` to read, and the envelope is the surface checked.
...
HEALTHY = {"status": "ok", "version": "0.12.0", "service": "juniper-cascor"}
ENVELOPE = {"/v1/workers": (200, {"status": "success", "data": {"workers": [], "count": 0}, "meta": {"version": "0.12.0"}})}
...
class TestEvaluate:
    ...
    def test_the_0110_image_fails(self):
        """The real 0.11.0 image: /v1/health right, every enveloped response stamped 0.6.0."""
        failures = _evaluate(
            expect="0.11.0",
            metadata="0.11.0",
            health_body={"status": "ok", "version": "0.11.0"},
            enveloped={"/v1/workers": (200, {"status": "success", "meta": {"version": "0.6.0"}})},
        )
        assert failures == ["/v1/workers meta.version 0.6.0 != metadata 0.11.0"], failures

    def test_no_module_means_no_module_check(self):
        """The image ships no juniper_cascor package; the envelope carries the version check instead."""
        assert _evaluate(module=None, module_version=None) == []
    ...
    def test_a_non_200_envelope_fails(self):
        failures = _evaluate(enveloped={"/v1/workers": (500, {"status": "error"})})
        assert any("/v1/workers answered 500" in f for f in failures), failures

    def test_an_image_that_is_not_the_tag_fails(self):
        failures = _evaluate(expect="0.12.1")
        assert any("installed metadata version 0.12.0 != expected 0.12.1" in f for f in failures), failures

    def test_missing_metadata_fails(self):
        failures = _evaluate(metadata=None, module_error="no distribution 'juniper-cascor'")
    ...
    def test_a_readiness_style_503_fails(self):
        """/v1/health/ready answers 503 standalone; the check must be pointed at liveness, and a 503 there fails."""
    ...
    def test_a_disagreeing_health_version_fails(self):
        failures = _evaluate(health_body={"status": "ok", "version": "0.6.0"})
        assert any("reports version 0.6.0 != metadata 0.12.0" in f for f in failures), failures
    ...
    def test_an_absent_module_version_fails_when_a_module_is_named(self):
        """Guards the shape the class-2 sweep scored as PASS, for any caller that names a module."""
        failures = _evaluate(module="cascade_correlation", module_version=None)
        assert any("has no __version__" in f for f in failures), failures


class TestUsage:
    @pytest.mark.parametrize("value", ["", "0.12", "v0.12.0", "latest", "0.12.0 "])
    def test_a_malformed_expected_version_is_a_usage_error(self, value):
        assert _load().main(["--image", "x", "--dist", "juniper-cascor", "--port", "8200", "--expect-version", value]) == 2
    ...
    def test_no_docker_is_an_environment_error_not_a_pass(self, monkeypatch):
        ...
        assert _load().main(["--image", "x", "--dist", "juniper-cascor", "--port", "8200", "--expect-version", "0.12.0"]) == 2
...
class TestPublishWorkflowRunsTheServeCheck:
    ...
    def test_both_steps_check_the_envelope_and_name_no_module(self, name):
        run = _step("build", name)["run"]
        assert "--dist juniper-cascor" in run and "--port 8200" in run
        assert "--enveloped-path /v1/workers" in run, "the envelope is the surface 0.11.0 got wrong"
        assert "--module" not in run, "the image ships no juniper_cascor package, so naming one would always fail"
    ...
    def test_the_pr_arm_runs_it_against_the_image_it_built(self):
        ...
        assert "cascor-smoke:${{ matrix.arch }}" in step["run"]
