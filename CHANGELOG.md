...
- **Docker image default now stays compatible with the SEC-F22 bind guard.** The runtime image no longer bakes in `JUNIPER_CASCOR_HOST=0.0.0.0` without `JUNIPER_CASCOR_FRONTING_AUTH_ATTESTED=true`, which would make a bare image crash-loop at lifespan startup. Published-container examples now show the explicit loopback host-publish plus attestation opt-in.
