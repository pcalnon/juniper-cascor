...
- **Worker result integrity — reject `success=True` without weights** (`src/api/workers/coordinator.py`, `src/api/workers/protocol.py`): `submit_result` previously accepted a successful `task_result` when `tensor_manifest` was empty/missing (skipping `validate_tensors`), so a worker could claim success with no `weights` tensor. Downstream `_dispatch_to_remote_workers` then rebuilt a `CandidateUnit` with random init weights, poisoning candidate selection. Successful results now require a non-empty `weights` tensor; `success=False` may still omit weights. `validate_tensors` also treats empty `weights` as a validation error instead of raising inside `np.max`. Covered by `test_worker_coordinator.py` / `test_worker_protocol.py`.
