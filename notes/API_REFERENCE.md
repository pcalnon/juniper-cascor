...
8. [Service Lifecycle API](#service-lifecycle-api)
...
**Location**: `src/api/lifecycle/monitor.py`, `src/api/lifecycle/manager.py`

### TrainingMonitor

Collects real-time training metrics and emits lifecycle events via callbacks.
...
Valid `event_type` values for `register_callback()`:
...
- Metrics are stored in an in-memory bounded buffer (`deque(maxlen=10000)`).
- `on_training_start()` clears prior metrics and resets epoch tracking.
- `on_epoch_end()` appends a metrics dict and triggers `epoch_end` callbacks.
- Callback exceptions are logged and do not stop training event flow.
- Retrieval is snapshot-based (`get_recent_metrics()` / `get_all_metrics()`).
- There is no blocking queue polling API; integrations should consume events via callbacks or read from the metrics buffer.
...
def on_epoch(metrics, epoch, loss, accuracy, **kwargs):
    print(f"epoch={epoch} loss={loss:.4f} acc={accuracy}")

monitor.register_callback("epoch_end", on_epoch)
...
latest = monitor.get_recent_metrics(1)[0]
print(latest["phase"])
```

### TrainingState

Thread-safe state container used by the lifecycle manager for status snapshots.
...
- Unknown keys in `update_state()` are ignored.
- Keys with `None` values are ignored (existing values remain unchanged).
- `timestamp` auto-updates when a valid field changes unless explicitly provided.

### Lifecycle Manager Integration

`TrainingLifecycleManager` owns one `TrainingMonitor` (`self.training_monitor`) and one `TrainingState` (`self.training_state`):

- `set_ws_manager()` wires monitor callbacks to WebSocket broadcasts.
- `get_metrics_history(count)` reads from the monitor buffer.
- `get_status()` merges state-machine, monitor, and training-state snapshots.
