...
|---------|-----------------|-----------------------|
| CI pip freeze | `conf/requirements_ci.txt` | Exact `twine==X.Y.Z` (Dependabot may bump; artifact also regenerated by `juniper-generate-dep-docs`) |
...
| Release upload | `pypa/gh-action-pypi-publish` | Twine bundled in the SHA-pinned action — independent of repo freezes |
...
| Twine freeze bump does not change upload behavior | Upload Twine is action-bundled; check jobs install Twine unpinned | See [Twine Pin Surfaces](#twine-pin-surfaces); only bumping the action SHA changes upload Twine |
