...
**Returns** — Envelope with: `training_state`, `training_active`, `network_loaded`, `state_machine`, `monitor`, `completion_reason`, `metrics_clear_undo_available`, `pending_dataset`, `current_dataset`, `dataset_shortfall`, `auto_start_failure`, `snapshot_seq`, `server_instance_id`. `metrics_clear_undo_available` (C5, additive) is `true` while an explicit metrics clear (`POST /v1/training/metrics/clear`) can still be undone — i.e. no run has started since — so a UI can render the undo affordance across a page reload without a separate poll. `dataset_shortfall` (additive) describes the LOADED dataset — the one a run trains on — and is `null` unless juniper-data could not produce that dataset in full, in which case it carries `dataset_id`, `accepted_by_this_run`, `acceptance_source`, `accepted_via_allow_truncated_datasets`, the producer's `truncation` / `data_quality` descriptors and a one-sentence `summary`; the same object is on `GET /v1/metrics`. It moves with the data (APD-CASCOR-013): a new fetch replaces it — a staged fetch, a live swap or the auto-start fetch, each of which replaces every partition — and a start that retains the data (a plain Stop → Start), a start-fresh or a `reset()` keeps it, so a run on partial data always says so, consistently with `current_dataset`. It also stays while ANY partition of that fetch is still loaded (owner ruling 2026-09-24, "keep while fetched splits stay"): a start that omits `val_x` / `val_y` or `test_x` / `test_y` keeps those partitions, so inline tensors that replace only some of them — `train_x` / `train_y` alone, or with one of the other two — leave the run early-stopping on, or reporting from, the fetch's rows, and this field and `current_dataset` keep naming that fetch. Raw inline tensors make it `null` (and `current_dataset` `{"dataset_type": null}`) only once all of the fetch's partitions have been replaced, in one start or across several. `auto_start_failure` (additive) is `null` normally and carries the reason the auto-start sequence gave up — auto-start swallows its failures so the service still comes up healthy, and this is the only surface that can be polled for why it did not train. `current_dataset` (additive) says WHICH dataset is loaded — the one the next start trains on unless `pending_dataset` replaces it — and has three readings a client must keep apart: `null` when nothing is loaded; `{"dataset_type": null}` when data is loaded but its identity is unknown (raw inline tensors); otherwise the config it was loaded from, `{"dataset_type": <name>, ...params}`, in the dialect `pending_dataset` uses. juniper-canopy reads it to hydrate its dataset selector after a page reload.
