...
- After admission, `task_result` acceptance is ownership-gated: `WorkerCoordinator.submit_result` rejects results when `worker_id != PendingTask.assigned_worker_id` (the task stays pending for the assignee). Tensor manifests are validated fail-soft via `WorkerProtocol.validate_tensors` (missing `shape`/`dtype`, non-dict entries, empty `weights` → errors, not crashes). Receive-site guards also reject non-object JSON and a malformed / over-32-entry `tensor_manifest` before binary receive. See [WS `/ws/v1/workers`](JUNIPER_CASCOR_API_REFERENCE.md#ws-wsv1workers).
- A remote `task_result` with `success=True` must carry a non-empty `weights` tensor (`WorkerCoordinator.submit_result`, checked ahead of `validate_tensors`); otherwise `_dispatch_to_remote_workers` would reconstruct a randomly initialized `CandidateUnit` and poison N-best selection.
- Four coordinator paths requeue an in-flight task **immediately**, none waiting for `JUNIPER_CASCOR_REMOTE_WORKERS_TASK_REASSIGNMENT_TIMEOUT` (default 120s): `_reject_and_requeue_task` (schema / tensor reject), `abort_in_flight_result` (soft binary-frame abort), `handle_worker_disconnect` (clean WebSocket close, from the session `finally`), and `requeue_after_dispatch_failure` (a `send_json` / `send_bytes` failure after the task was already assigned). Each emits its own distinct requeue log line. Operator detail: [JUNIPER_CASCOR_API_REFERENCE.md — WS `/ws/v1/workers`](JUNIPER_CASCOR_API_REFERENCE.md#ws-wsv1workers).
- `WorkerCoordinator.cancel_round` captures every worker still holding an in-flight assignment before clearing the coordinator maps, then frees each via `registry.complete_task(..., success=False)` — otherwise those workers stay busy permanently, since `_check_task_timeouts` can no longer see the cleared pending tasks.
